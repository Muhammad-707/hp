...
export function WishesSlider() {
  ...
  return (
    <section className="py-24 px-4 relative flex flex-col justify-center items-center">
      ...
      >
        <h3 className="text-3xl md:text-4xl font-serif text-rose-100 mb-8">
          {t.wishes.title}
        </h3>

        <Tabs defaultValue="wish-0" className="w-full">
          <TabsList className="mb-8 flex-wrap h-auto justify-center p-2 rounded-2xl bg-black/30 backdrop-blur-sm border border-rose-900 shadow-sm">
            {t.wishes.items.map((_, index) => (
              <TabsTrigger 
                key={index} 
                value={`wish-${index}`}
                className="rounded-xl px-4 py-2 m-1 text-rose-300 data-[state=active]:bg-rose-500 data-[state=active]:text-white"
              >
                {index + 1}
              </TabsTrigger>
            ))}
          ...
          {t.wishes.items.map((wish, index) => (
            <TabsContent key={index} value={`wish-${index}`}>
              ...
              >
                <Card className="bg-gradient-to-br from-rose-800 to-rose-950 border-0 shadow-2xl text-white transform hover:-translate-y-2 transition-transform duration-300">
